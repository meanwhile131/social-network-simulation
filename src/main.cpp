...
void runSimulation(string* stats, unsigned int run, size_t iterations, size_t edges_per_iter, float choose_neighbor_chance) {
    ...
    for (size_t i = 0; i < iterations; i++)
    {
        ...
        transform(nodes.begin(), nodes.end(), back_inserter(weights), [](const auto& node) { return node.size(); }); // Записывает веса (степени) вершин в weights
        ...
        if (i % 1000 == 1)
        {
            ...
            for (auto node1_i : nodes[0])
            {
                for (auto node2_i : nodes[node1_i])
                {
                    ...
                    path2_count++;
                    if (find(node2.begin(), node2.end(), 0) != node2.end()) // У вершины 0 есть связь с этой -> треугольник
                        triangle_count++;
                }
            }
            cout << run << "\t" << i << "\n";
